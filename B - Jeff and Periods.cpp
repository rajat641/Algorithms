...
//	freopen("inp.txt","r",stdin);
  ...
  for(map< int , vector< int > >::iterator i=mm.begin();i!=mm.end();i++){
  	if((i->second).size()==1){
  		ans.push_back(make_pair(i->first,0));
  	...
  	if(ch(ss)){
  		...
  		ans.push_back(make_pair(i->first,d));
  		d=-1;
  	}	
