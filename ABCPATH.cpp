/*
RAJAT SINGH
VIT VELLORE
*/
 
#include<bits/stdc++.h>
using namespace std;
#define ll   long long 
//#define max 706000
#define gc getchar_unlocked
#define pc putchar_unlocked
//inline void get(int *p){register  int n=0,sign=1,a=gc();while(a<'0'||a>'9'){ if(a=='-'){sign=-1;a=gc();break;} a=gc();}while(a>='0' && a<='9'){n=(n<<1)+(n<<3)+a-'0';a=gc();}*p=n*sign;}
  
 
//inline void put(int n){char d[10];int i=0;if(n<0){pc('-');n=-n;}do{d[i++]=n%10+'0';n=n/10;}while(n);while(i)pc(d[--i]);pc('\n');}
#define     sc(a)  scanf("%d ",&a) 
#define     p(a)  printf("%lld\n",a) 
#define     l(i,a)  for(int i=0;i<a;i++)
#define     re(j,a,b)  for(int j=a;j<=b;j++)  
# define ll   long long 
#define modu 1000000007
ll  modular_pow(ll int  base, ll int  exponent)
{
    if(exponent==0)
    return 1;
    else if(exponent%2==0){
        ll int res=modular_pow(base,exponent/2);
        return (res*res );
        
        
    }
    else{
        return (modular_pow(base,exponent-1) * base);
        
    }
}
 
 
 
 int gcd(int a,int b){
    if(b==0)
    return a;//gcd
    else return gcd(b,a%b);
 }
 
bool chek(int x){
    return (x&&!(x&(x-1)));// power of 2
}
int onebits(int n){
    return __builtin_popcount(n);//count number od set bits in a number
 
}
char a[100][100];
int visit[100][100];
int mm=0;
int row;
int col;
void ser(char x, int dis, int r, int c){
	if(r< 0 || r>=row || col<0 || c>=col && !visit[r][c]){
		mm=max(mm,dis);
		//visit[r][c]=1;
		return ;
	}
	else{
		if(a[r-1][c]==x+1 && visit[r-1][c]==0){
		visit[r-1][c]=1;
		ser(x+1,dis+1,r-1,c);}
		
		if(a[r+1][c]==x+1 && visit[r+1][c]==0){
		visit[r-1][c]=1;
		ser(x+1,dis+1,r+1,c);}
		
		if(a[r][c-1]==x+1 && visit[r][c-1]==0){
		visit[r][c-1]=1;
		ser(x+1,dis+1,r,c-1);}
		
		 if(a[r][c+1]==x+1 && visit[r][c+1]==0){
		 visit[r][c+1]=1;
          ser(x+1,dis+1,r,c+1);}
         
		 if(a[r-1][c-1]==x+1 && visit[r-1][c-1]==0){
		 visit[r-1][c-1]=1;
		  ser(x+1,dis+1,r-1,c-1);}
		 
		 if(a[r-1][c+1]==x+1 && visit[r-1][c+1]==0){
		 visit[r-1][c+1]=1;
		ser(x+1,dis+1,r-1,c+1);}
		 
		if(a[r+1][c-1]==x+1 && visit[r+1][c-1]==0){
		visit[r+1][c-1]=1;
		ser(x+1,dis+1,r+1,c-1);}
		
		if(a[r+1][c+1]==x+1 &&  visit[r+1][c+1]==0){
		 visit[r+1][c+1]=1;
		 ser(x+1,dis+1,r+1,c+1);}
		 
		else{
			mm=max(mm,dis);
			return;
		}
		
		
	}
	
}

int main(){  
int k=1;
//freopen("input.txt","r",stdin);
while(1){
bool fl=false;
scanf("%d %d",&row, &col);
if(row==0 && col==0)
break;
l(i,100){
	l(j,100){
	 a[i][j]='a';
	 visit[i][j]=0;}
}
l(i,row){
	l(j,col) cin>>(a[i][j]);
}
l(i,row){
	l(j,col){
		if(a[i][j]=='A'){
	       visit[i][j]=1;
		   fl=true;	
		ser('A',0,i,j);}
	}
}

if(fl)
printf("Case %d: %d\n",k,mm+1);
else printf("Case %d: %d\n",k,mm-mm);
//cout<<mm+1<<endl;
k++;
mm=0;}
	return 0;
} 

